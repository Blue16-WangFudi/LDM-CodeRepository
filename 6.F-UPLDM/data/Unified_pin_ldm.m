function [margin_mean,margin_variance, acc_f, C_f] = Unified_pin_ldm(X_train, Y_train, X_test,Y_test, kernel, tau, c1val,p1,lamb1, lamb2)
%%未做归一化处理
[m, n] = size(X_train);
H = zeros(m,m);
m1=size(X_test,1);
...
if (kernel == 1)
    ...
    K = [Function_Kernel(X_train, X_train,'RBF',p1), e]';  %王老师中的G
...
if (kernel == 1)
    ...
    alpha = omega;
    G = Function_Kernel(X_train, X_train,'RBF',p1);
    margin_mean = Y_train'*G*alpha/m;
    margin_variance = 2*(m*alpha'*G*G*alpha-alpha'*G*Y_train*Y_train'*G*alpha)/(m^2); 
end
