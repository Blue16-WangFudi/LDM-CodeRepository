...
for jj=1:length(tauval)
        ...
        time2 = time2 + toc;
        tic
        [acc3,C3] = Unified_pin_fsvm(Ctrain, dtrain, Ctest,dtest, kernel, 0,C,p1,s); %FUPSVM(When tau=0, it turns to FSVM)
        time3 = time3 + toc;
...
[acc_svm,i0]=max(acc0_); st_svm=std(acc0_);
[acc_upsvm,i1]=max(acc1_); st_upsvm=std(acc1_);
[acc_psvm,i2]=max(acc2_); st_psvm=std(acc2_);
[acc_fupsvm,i3]=max(acc3_); st_fupsvm=std(acc3_);
[acc_upldm,i4] = max(acc4_); st_upldm=std(acc4_);
[acc_fupldm,i5] = max(acc5_); st_fupldm=std(acc5_);
[acc_ldm,i6] = max(acc6_); st_ldm=std(acc6_);
[acc_fldm,i7] = max(acc7_); st_fldm=std(acc7_);
opt_tau1=tauval(i1);
opt_tau2=tauval(i2);
opt_tau3=tauval(i3);
opt_tau4=tauval(i4);
