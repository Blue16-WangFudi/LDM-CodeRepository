function Outs_Train = Train_LDM(Samples_Train, Labels_Train, lambda1, lambda2, C, rou,  Kernel, QPPs_Solver)
...
%% Main 
  m1 = length(find(Labels_Train==-1));
  m2 = length(find(Labels_Train==1));
  D = zeros(length(Labels_Train));
  D2 = zeros(length(Labels_Train),1);
  for rr = 1:length(Labels_Train)
      if Labels_Train(rr)==1
          sita = (m1/m2)^rou;
      else
          sita = (m2/m1)^rou;
      end
...
%            H = GY'*(Q\GY);
         ...
         % solver
           beta = quadprog(H, z, Labels_Train', -lambda2*e'*Labels_Train/m, [], [], zeros(m, 1), C.*D2, beta0, Options);  %二次规划，内置函数求解。标准形式为：min1/2xTHx+fTx
%            alpha = Q\(GY*(lambda2*e/m+beta));
           alpha = inv(Q)*GY*(lambda2*D*e/m+beta);
       case ' CD_LDM'
