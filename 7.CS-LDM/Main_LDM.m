...
%% Some parameters
  ...
  %% Counts
     ...
     switch Kernel.Type
         case 'Linear'
             Stop_Num = N_Times*length(lambda1_Interval)*length(lambda2_Interval)*length(C_Interval)*length(rou_Interval)*K_fold + 1;
         case 'RBF'
             Stop_Num = N_Times*length(lambda1_Interval)*length(lambda2_Interval)*length(C_Interval)*length(gamma_Interval)*length(rou_Interval)*K_fold + 1;
         otherwise
...
%             end   %  lambda2
      ...
      % Train with the best parameters
        ...
        m3 = length(find(Labels_Predict==-1));
...
%% Statistical results 
    ...
    disp(['  The rou_leader is ', num2str(rou_Leader), '.'])
    if strcmp(Kernel.Type, 'RBF')
...
%% Visualization
 ...
 % The Intervals for both X and Y axise
  ...
  % Contours
    [X, Y, Z] = Contour_LDM(Outs_Leader, x_Interval, y_Interval); 
