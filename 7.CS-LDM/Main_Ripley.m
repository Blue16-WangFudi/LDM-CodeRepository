...
%% Load and prepare the data
 % Training data
   ...
   DataTrain_Name = fieldnames(Output_Train);   % A struct data
    Data_Train = Output_Train.(DataTrain_Name{1}); % Use dot notation to abstract the data
 ...
 % Predicting data
  ...
  Data_Predict = Output_Predict.(DataPredict_Name{1}); % Use dot notation to abstract the data
                ...
                for ith_gamma = 1:length(gamma_Interval)       %   gamma
                    ...
                    for repeat = 1: K_fold
                      ...
                      %%%%%%-------Computes the average distance between instances-------%%%%%%
                        ...
                        Outs_SubTrain = Train_LDM(Samples_SubTrain, Labels_SubTrain, lambda1, lambda2, C*abs(Labels_SubTrain),0.5,Kernel, QPPs_Solver);
...
%% Main
  % Train the data with best parameters
    ...
    tic
    Outs_Train = Train_LDM(Samples_Train, Labels_Train, Best_lambda1, Best_lambda2, Best_C*abs(Labels_Train), 0.2,Best_Kernel, QPPs_Solver);
    t = toc;
  % Predict the data
...
%% Visualization
   ...
   Margin_Histc = histcounts(Margin_SAMPLES, Margin_Unique);
   Margin_Cumsum = cumsum(Margin_Histc)/length(Labels_Train);
   plot(Margin_Unique(1:249), Margin_Cumsum, 'k');
   legend(Str_Legend)
   figure(2)
...
try
    ...
    disp('An error occurred while trying to plot contours:');
    disp(err.message);
end
