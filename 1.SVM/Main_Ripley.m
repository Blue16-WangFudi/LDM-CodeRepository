...
%% Some parameters
  ...
  %% Some statistical variables
     ...
     switch Kernel.Type
         case 'Linear'
             Stop_Num = length(C_Interval)*K_fold + 1;
         case 'RBF'
             Stop_Num = length(C_Interval)*length(gamma_Interval)*K_fold + 1;
         otherwise
...
%                 Kernel.gamma = delta_Sub*gamma_Interval(ith_gamma);  %   gamma
                ...
                Stop_Num = Stop_Num - 1;
                disp([num2str(Stop_Num), ' steps(s) remaining.'])
...
%% Visualization
 ...
 % Contours
   [X, Y, Z] = Contour_SVM(Outs_Predict, x_Interval, y_Interval); 
