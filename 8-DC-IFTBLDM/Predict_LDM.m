...
%   2.Outs_Train.alpha      the solution to primal problem %alpha=w
% 3.Outs_Train.Samples      the samples
%  4.Outs_Train.Labels      the cooresponding labels of Samples
%       5.Outs_Train.C      the parameter for slack variables  松弛变量参数
%     6.Outs_Train.Ker      kernel type

%      Samples_Predict      the samples for predicting
%         bels_Predict      the cooresponding labels of the Samples for predictin
...
%% Main
 ...
 %------------Margin statistics------------%
   ...
   Margin.SAMPLES = diag(Labels_Train)*G*alpha;%样本间隔
   Margin.MEAN = Labels_Train'*G*alpha/m;%间隔均值
   Margin.VARIANCE = 2*(m*alpha'*G*G*alpha-alpha'*G*Labels_Train*Labels_Train'*G*alpha)/(m^2); %间隔方差
