function [acc,outclass,time, fp, fn]= tbsvmclass(ftsvm_struct,Testdata,Testlabel)
...
if ( nargin>3||nargin<2) % check correct number of arguments
    ...
    switch ftsvm_struct.Parameter.ker
        case 'linear'
            ...
            fn=(Testdata*vn(1:(length(vn)-1))+vn(length(vn)))./norm(vn(1:(length(vn)-1)));%f(x)=srg min(+_)
        case 'rbf'
