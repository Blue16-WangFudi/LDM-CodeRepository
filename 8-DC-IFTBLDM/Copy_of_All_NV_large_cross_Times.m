...
switch Kernel.Type
    ...
    case 'RBF'
        Stop_Num = 2*N_Times*K_fold*length(lambda1_Interval)*length(lambda2_Interval)*length(C1_Interval)*length(C3_Interval)*length(gamma_Interval) + 1;
    otherwise
...
%         dtest= test(:,end);
    ...
    if (iData==27)
        load('Data_mat_x\transfusion.mat');
        X = transfusion;
...
%                                                     %%%%%%-------Computes the average distance between instances-------%%%%%%
                                            ...
                                            [Subtbsvm_Substruct_] = DC_IFtbsvmtrain_New(Samples_SubTrain,Labels_SubTrain,Parameter_);
                            ...
                            tic         
                            [tbsvm_struct_] = DC_IFtbsvmtrain_New(Samples_Train,Labels_Train,Best_Parameter_);
...
%                                         C_s.s2 = s.s2;
        ...
        %                     BestC_s.s2 = Bests.s2;
                            ...
                            Bests_ = DC_Fuzzy_MemberShip(Samples_Train, Labels_Train, Best_Kernel_, Best_u); 
...
%             
                ...
                xlswrite('result.xlsx',100*(C3_ACCp_(1,:)),['C',num2str(iData+339),':O',num2str(iData+339)]);
                xlswrite('result.xlsx',100*(C3_ACCp_(2,:)),['Q',num2str(iData+339),':AC',num2str(iData+339)]);
                xlswrite('result.xlsx',100*(C3_ACCp_(3,:)),['AE',num2str(iData+339),':AQ',num2str(iData+339)]);
                xlswrite('result.xlsx',100*(C3_ACCp_(4,:)),['AS',num2str(iData+339),':BE',num2str(iData+339)]);
                xlswrite('result.xlsx',100*(C3_ACCp_(5,:)),['BG',num2str(iData+339),':BS',num2str(iData+339)]);
                xlswrite('result.xlsx',100*mean(C3_ACCp_),['BU',num2str(iData+339),':CG',num2str(iData+339)]);
                
                xlswrite('result.xlsx',100*Acc_Predict_',['CI',num2str(iData+339),':CM',num2str(iData+339)]);    %xlswrite只能写入行向量，如果是列向量，则只写入第一个元素，所有都是相同的             
