function s = DC_Fuzzy_MemberShip_aaaa(Data, Label, Kernel, u)
...
%    delta_Neg = 0.1*s_s;
   ...
   d_p = P_N - P_P;% 由于三角形任意两边之差大于第三边，C_pn永远是大于等于d_p中的值！！！
   s_Pos = (d_p - min(d_p)+10e-7)./((C_pn - min(d_p)));%分子加 10e-7 避免出现 0 
   
   % Compute the membership of negative data
   d_n = N_P - N_N;% 由于三角形任意两边之差大于第三边，C_pn永远是大于等于d_p中的值！！！
   s_Neg = (d_n - min(d_n)+10e-7)./((C_pn - min(d_n)));
