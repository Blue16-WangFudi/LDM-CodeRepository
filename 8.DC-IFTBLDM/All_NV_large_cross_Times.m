...
%                                                             membership(Samples_Train', Labels_Train')
                                                            s = Fuzzy_MemberShip(Samples_SubTrain, Labels_SubTrain, Kernel, Best_u); 
                                                            C_s.C1 = C1;
                                                            C_s.C2 = C2;
                                                            C_s.s1 = s.s1;
                                                            C_s.s2 = s.s2;
...
%                 Name = Str_Name(10:end-4);
                ...
                xlswrite('result.xlsx',100*(C3_ACCp(1,:)),['C',num2str(iData+380),':O',num2str(iData+380)]);
                xlswrite('result.xlsx',100*(C3_ACCp(2,:)),['Q',num2str(iData+380),':AC',num2str(iData+380)]);
                xlswrite('result.xlsx',100*(C3_ACCp(3,:)),['AE',num2str(iData+380),':AQ',num2str(iData+380)]);
                xlswrite('result.xlsx',100*(C3_ACCp(4,:)),['AS',num2str(iData+380),':BE',num2str(iData+380)]);
                xlswrite('result.xlsx',100*(C3_ACCp(5,:)),['BG',num2str(iData+380),':BS',num2str(iData+380)]);
                xlswrite('result.xlsx',100*mean(C3_ACCp),['BU',num2str(iData+380),':CG',num2str(iData+380)]);
                
                xlswrite('result.xlsx',100*Acc_Predict',['CI',num2str(iData+380),':CM',num2str(iData+380)]); %xlswrite只能写入行向量，如果是列向量，则只写入第一个元素，所有都是相同的                   
